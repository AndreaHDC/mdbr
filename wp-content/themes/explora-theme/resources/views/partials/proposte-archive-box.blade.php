<div class="border border-black rounded-tr-xl rounded-tl-xl overflow-hidden mb-6">
    ...
    <div class="p-6">
        @if (isset($event['terms']) && $event['terms'])
            <ul class="flex gap-3 mb-3 text-sm">
                @foreach ($event['terms'] as $term)
                    <li class="inline-block px-2 py-1 bg-exp-yellow-300 rounded-md uppercase">{{$term->name}}</li>
                @endforeach
            </ul>
        @endif
        <span class="block mb-3 uppercase text-lg">{{$event['date_text']}}</span>
